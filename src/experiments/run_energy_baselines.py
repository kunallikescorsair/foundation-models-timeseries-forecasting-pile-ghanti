"""
Run baseline models on the selected energy datasets.

This script currently runs:
- Naive
- Seasonal Naive

It is intentionally conservative for high-frequency datasets.
"""
...
from src.data.project_config import ENERGY_DATASETS
...
def run_energy_baselines(
    output_dir: str | Path = "results/tables/energy_baselines",
    ...
    """
    Run baseline models on selected energy datasets.
    ...
    for dataset_key in ENERGY_DATASETS:
        print(f"\nProcessing energy dataset: {dataset_key}")
...
if __name__ == "__main__":
    all_results_df, summary_df, run_status_df = run_energy_baselines(series_limit=20)
